...
{ // Функция сохранения фулл текста файла без нужной структуры
    string str, text;
    ifstream fin;
    fin.open(filenm);

    while (getline(fin, str))
    { // Сохранение фулл текста в переменную
        stringstream ss(str);
        string tokens;
        getline(ss, tokens, ' ');
...
Array hReadFile(string &filenm, string &nameStruct)
{
    ...
    while (getline(fine, str))
    {
        ...
        getline(ss, tokens, ' ');
        if (tokens == nameStruct)
        { // Проверяем на совпадение с нужной структурой
...
void MREPLACE(string &name, size_t index, string &filenm)
{
    string textfull = Futext(filenm, name);
    Array arr = hReadFile(filenm, name);
    if (index >= arr.getSize())
...
void aMenu(string &command, string &filenm)
{
    ...
    else if (command.substr(0, 9) == "MREPLACE")
    {
        ...
        stringstream stream(cons);
        stream >> name >> value >> index;
        MREPLACE(name, index, value, filenm);
    }
...
void lMenu(string &command, string filenm)
{
    ...
    else if (command.substr(0, 7) == "LPUSH ")
    {
        string cons = command.substr(7);
        stringstream stream(cons);
        stream >> name >> value;
    ...
    else if (command.substr(0, 6) == "LPOP ")
    {
        string cons = command.substr(6);
        stringstream stream(cons);
        stream >> name;
...
{ // Функция обработки команд стека
    string name, value;
...
// Хеш-таблицы
Hash_table hReadFile(string &filenm, string &name)
{ // ф-ия чтения Хеш-таблицы из файла
    Hash_table nums;
    string str;
    ifstream str;
    fin.open(filenm);
...
string printHashTable(Hash_table &hashtable, string name)
{ // Функция для перебора всех элементов хеш-таблицы
    string textfull = Futext(filenm, name);
    Hash_table nums = hReadFile(filenm, name);
...
void HPOP(string *name, string &key, string &filenm)
{
    ...
    {
        Hash_table data = hReadFile(filenm, name);
    ...
    void hMenu(string & command, string & filenm)
    { // ф-ия обработки команд Хеш-таблицы
